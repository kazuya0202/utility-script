...
def parser():
    weights_path = r".\backup\yolov4-obj_" + str(save_number) + ".weights"
    config_path = r".\config\yolov4-obj.cfg"
    data_path = r".\config\obj.data"
...
def batch_detection(
    network, images, class_names, class_colors, thresh=0.25, hier_thresh=0.5, nms=0.45, batch_size=4
):
...
def batch_detection_example():
    ...
    (
        images,
        detections,
    ) = batch_detection(network, images, class_names, class_colors, batch_size=batch_size)
...
def main():
    ...
    for ext in exts:
        images = [x for x in p.glob(f"*.{ext}")]
        out = Path(f"./predictions-results_{save_number}")
        out.mkdir(parents=True, exist_ok=True)

        for x in images:
            image, detections = image_detection(
                str(x), network, class_names, class_colors, args.thresh
            )
            cv2.imwrite(str(out.joinpath(x.name)), image)
            print(detections)
    exit()
